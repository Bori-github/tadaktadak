...
- 숫자는 남은 초가 바뀔 때 `runOnJS`로 JS 스레드에 넘겨 다시 그린다. 도트 격자가 글자마다 달라 UI 스레드에서 바꿀 수 없다. 완료 같은 상태 전이도 같은 방식으로 넘긴다
...
- 완료에서는 모드를 지우지 않는다. 5초 뒤 휴식 타이머를 시작할 때 무엇이 끝났는지 알아야 한다
