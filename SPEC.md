...
- 시작할 때 끝날 시각을 `지금 시각 + 설정 시간`으로 잡는다. 알림과 Live Activity에 이 시각을 넘긴다
- 진행 중에는 매 프레임 `설정 시간 − 시작 후 흐른 시간`으로 남은 시간을 새로 구한다. 흐른 시간은 기기 가동 시간(`performance.now`)으로 잰다. 이전 프레임 값을 이어받지 않는다
- 계산은 UI 스레드에서 한다. `react-native-reanimated`의 `useFrameCallback`이 프레임마다 남은 시간을 shared value에 쓰고, 그리기는 그 값을 읽는다. 완료 같은 상태 전이만 `runOnJS`로 JS 스레드에 넘긴다
- 호, 손잡이 각도, 켜진 개체, 차오름 진행률, 숫자는 전부 남은 시간에서 계산한다
...
- 진행 상태로 바뀔 때(재생·재개) 알림을 예약하고 Live Activity를 시작하고 화면 꺼짐 방지를 켠다
- 진행 상태에서 벗어날 때(일시정지·완료·정지) 알림을 취소하고 Live Activity를 끝내고 화면 꺼짐 방지를 끈다
...
| 진행 중에 종료됐고 그 사이 끝남     | 끝날 시각 (지금보다 앞) | 완료 상태                                         | 끝냄          |
| 대기 중에 종료됨, 또는 첫 실행      | 없음                    | 집중 타이머 대기. 설정 분은 저장값, 없으면 기본값 | 끝냄          |

## 알림과 감각 피드백

- 타이머마다 끝나는 시각에 로컬 알림 하나를 예약한다. 집중 타이머와 휴식 타이머 양쪽 다
...
앱을 처음 켤 때 한 번 요청하고, 켤 때마다 권한 상태를 확인한다.
...
- 조작 버튼 아래에 문구를 띄운다. 탭하면 시스템 설정의 앱 페이지를 연다
