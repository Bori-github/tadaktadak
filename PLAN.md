...
- [ ] 시간 모델. 아래 순서로 만든다.
  ...
  - [ ] 저장값 쓰기와 앱 재실행 연결. 훅이 언제 저장하는지 보고 만든다
  - [ ] 호와 손잡이를 UI 스레드로. 진행 중 손잡이 색은 `DESIGN.md` §8대로 크림. `useDialDrag`의 제스처를 `useMemo`로 감싼다
  ...
  - [ ] 개체 점화와 차오름. 타는 프레임 격자를 시안에서 옮기고 `DialItems`를 Skia `Atlas` 하나로 바꾼다
  - [ ] 휴식 타이머 자동 시작. 완료 → 5초 → 휴식 → 집중 대기
  - 알림과 Live Activity는 각자의 단계에서 상태 전이에 더한다
- [ ] 개발 빌드 배속. 타이머 시간을 5~50배로 돌린다. 조작은 `__DEV__`에서만 보이고 대기에서만 바뀐다
- [ ] 불꽃, 불티, 스파크 반짝임, 빛 번짐, 완료 연출. 빛 번짐은 반지름별 텍스처를 앱 시작 때 한 번 만들고 알파·스케일만 바꿔 그린다
- [ ] 소리, 진동
...
- [ ] 검증
  ...
  - [x] 편집 대상별 손잡이 색 — 집중 크림, 휴식 청록 (화면 조립)
    - 호 색은 Jest에 `CanvasKit`이 없어 `Skia.PathBuilder`가 빈 채로 온다. 화면으로 확인한다
  - [ ] 손잡이를 잡는 범위 게이트 — 손잡이 밖을 잡으면 끌어도 시간이 바뀌지 않는가. `react-native-gesture-handler/jest-utils`의 `fireGestureHandler` (단위)
    - 제스처에 `.withTestId()`와 하니스 컴포넌트가 필요하다
  ...
  - [ ] 완료에서 다음으로 — 휴식 타이머가 있으면 휴식 진행, 0분이면 집중 타이머 대기 (단위)
...
| `expo-audio`                                | 57.0.4  | 소리와 오디오 세션                     | 포함됨                |
