...
- 피그마 프로토타입에 없는 동작은 `design/prototype.html`에서 확인한다
- 원본을 프로토타입에서 피그마로 옮기는 중이다. 완료되기 전까지 `DESIGN.md`, `design/prototype.html`, 피그마를 모두 함께 고려한다
...
- 시안에 걸리는 결정이면 `design/prototype.html`도 같은 커밋에서 고친다
